...
Element.NativeEvents = {
	...
	focus: 2, blur: 2, change: 2, reset: 2, select: 2, submit: 2, DOMFocusOut: 2, //form elements
	...
	condition: function(e){
		switch(e.key){
			case 'up': case 'down': case 'left': case 'right':
				...
				if(type == 'radio') return this.checked;
				if(type == 'select') return (this.retrieve('change:value') != this.value) ? this.store('change:value', this.value) : false;
			break;
			
			case 'space': return getType(this) == 'checkbox'; break;
		}
	}
};
...
Element.Events.change = {
	base: function(){
		var type = getType(this);
		return (type == 'checkbox' || type == 'radio' || type == 'select') ? 'mouseup' : 'change';
	},
	condition: function(event){
		var type = getType(this);
		if(type == 'select') return (this.retrieve('change:value') != this.value) ? this.store('change:value', this.value) : false;
		return (type == 'radio') ? (event.type == 'keyup' ? this.checked : !this.checked) : true;
	},
	onAdd: function(fn, base){
		var type = getType(this);
		if (base == 'mouseup' || type == 'select') this.addEvent('keychange', fn);
		if(type == 'select') this.store('change:value', this.value)
	},
	onRemove: function(fn, base){
		(base == 'mouseup' || getType(this) == 'select' ? this.removeEvent('keychange', fn) : this).removeEvent(base, fn);
	}
}
