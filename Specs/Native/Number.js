...
describe('Number.times | Number.each', {
...
(function(math){
	new Hash(math).each(function(value, key){
		var example = {};
		var b = value.test[1];
		example['should return the ' + value.title + ' value of the number' + ((b) ? ' and the passed number' : '')] = function(){
			value_of(value.test[0][key](b)).should_be(Math[key].apply(null, value.test));
		};
		describe('Number.' + key, example);
	});
})({
	abs: {
		test: [-1],
	...
	ceil: {
		test: [0.6],
		title: 'number closest to and not less than the'
	},
	cos: {
		test: [30],
		title: 'cosine'
	...
	floor: {
		test: [2.4],
		title: 'integer closet to and not greater than'
	},
	log: {
		test: [2],
		title: 'log'
